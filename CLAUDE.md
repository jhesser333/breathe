...
- Navigation: Select Mode (home) → Slider Layouts (top-left) or Art Options (bottom-center "Change the Art" button, shows the Shape Options picker directly); Select Mode → Start button (launches the selected mode).
- All nav buttons are consolidated into stacked bottom-center groups — no top-corner buttons on any screen except Select Mode's **Slider Layouts** (top-left) button and the Diagonal experience-screen nav (see below). Gap between stacked buttons: 20px. Positioned at `bottom: 16, left: '50%', transform: 'translateX(-50%)'`.
...
- **`BackgroundB`** (Shape Option E only): 5 concentric torus rings (default/unrotated orientation, facing the camera like Gates torus meshes), sharing one material (`color: gateColor`, **no emissive**, `transparent`, `opacity: 0.6`), positioned in a group at `[0, 0.25, -2]` with radii stepped 0.6→2.4. Unlike `BackgroundA`'s per-mesh opacity/Y animation, the entire group's `scale` lerps 0→1 (0 at Inhale, full size at Exhale, matching the concept art), reusing `BackgroundA`'s exact `progressRef`/`target`/`halfInterval`/local-smoothstep timing logic (driven by the same `breathPhaseRef`/`gatesEnabledRef`/`spawnIntervalRef` props). The outer rings are expected to bleed past the screen edges on a portrait phone at full scale — intentional, matching the reference art — but the starting radius/position constants are a first pass meant to be tuned on-device.
...
```
src/
  App.jsx                   — screen routing (selectMode [home, initial state]/sliderLayouts/personalize/shape, else falls through to the experience screen), shape/sliderLayout state (shapeOption defaults to `'d'` for first-time users, see localStorage init; whitelist is `['a','b','c','d','e']`), tutorial logic, localStorage; palette is locked to `PALETTES.a` (no colorPalette state, no picker); tutorial architecture uses pendingGatesFnRef (function thunk) for mode-specific Text C; key refs: recordingEnabledRef, lastMaxTimeRef, gateEnableTimerRef, shapeRef, bbCycleRef, bbTutorialActiveRef, breathPhaseRef (owned here, written by gate files on Z=0 crossings, read by BackgroundA/BackgroundB), holdFlareRef (owned here, written by GatesBoxBreathingHeadlessE, read by StarFieldE); backgroundOption is a derived constant (not stored state): `shapeOption === 'e' ? 'b' : shapeOption === 'd' ? 'a' : 'none'`; handleBBFirstGate sets breathPhaseRef.current before tutorial guard so BackgroundA/BackgroundB respond during Box Breathing post-tutorial; showTimedText / showSlowingTextC–G / handleSlowingRecordingDone / handleSlowingTextDDone–GDone / handleSlowingRampDone / handleSlowingShowSlider / showBoxText / transitionBoxText / handleBBFirstGate / handleBBLastGate; showGatesText/showSlowingTextD/showSlowingTextE branch on `shapeRef.current === 'd' || shapeRef.current === 'e'` to pick ambient copy variants; renders a persistent top-center `MODE_LABELS[mode]` caption in the experience-screen overlay for the duration of play (see Modes section)
  ...
  SelectModeScreen.jsx      — app's home screen (HOME title, smaller "Modes" subheading, 4 selectable mode cards labeled via MODE_LABELS, checkmark/highlight state, default 'basic'); Slider Layouts nav button top-left, bottom-center stacked Start (launches selected mode)/Change the Art (→ Art Options screen) buttons
  ...
  BackgroundB.jsx           — 5 concentric torus rings (unrotated, camera-facing), sharing one no-emissive material (gateColor, opacity 0.6); whole-group scale (not per-mesh opacity/Y) lerps 0→1 via the same progressRef/target/halfInterval/smoothstep pattern as BackgroundA; props: gateColor, breathPhaseRef, gatesEnabledRef, spawnIntervalRef; rendered only when shapeOption === 'e', in every mode including Box Breathing
