...
- Navigation: Select Mode (home) → Slider Layouts (top-left) or Colors (top-right) or Art Options (bottom-center "Change the Art" button, shows the Shape Options picker directly); Select Mode → Start button (launches the selected mode).
- All nav buttons are consolidated into stacked bottom-center groups — no top-corner buttons on any screen except Select Mode's **Slider Layouts** (top-left) and **Colors** (top-right) buttons and the Diagonal experience-screen nav (see below). Gap between stacked buttons: 20px. Positioned at `bottom: 16, left: '50%', transform: 'translateX(-50%)'`.
  ...
  - **Color Options screen**: **Personalize** / **Select Mode** / **Resume Breathing** — its "Personalize" nav button label is unchanged and still routes to the (now ART OPTIONS-titled) Art Options screen.
  - On the Color Options screen, the **Select Mode** button (prop `onSelectMode`, was `onBack`/`onHome`) jumps straight to the Select Mode (home) screen. Its **Personalize** nav button returns to the Art Options screen, which shows the Shape Options picker directly.
- Art Options and Color Options screens use a scrollable cards container (`height: 380, overflowY: 'auto'`) — shows ~4 cards at a time; scroll to reveal more. Layout is `justifyContent: 'flex-start'` with `paddingTop: 60, paddingBottom: 150` so cards don't slide under the bottom button group.
...
src/
  ...
  SelectModeScreen.jsx      — app's home screen (HOME title, smaller "Modes" subheading, 4 selectable mode cards labeled via MODE_LABELS, checkmark/highlight state, default 'basic'); Slider Layouts nav button top-left, Colors nav button top-right (same pillStyle, symmetrical), bottom-center stacked Start (launches selected mode)/Change the Art (→ Art Options screen) buttons
  ...
  PersonalizeScreen.jsx     — Art Options screen (on-screen title "ART OPTIONS", screen id stays 'personalize'); shape selection (A–D) shown directly, scrollable card list (height 380, overflowY auto), single Home nav button at bottom (Colors moved to SelectModeScreen)
