...
- **Diagonal layout Text A1/A2/B1/B2** (replaces Text A/B entirely for `sliderLayout === 'diagonal'`; driven by `updateDiagonalSequence` in `App.jsx`, called from `setLeft`, using `diagStageRef` as the stage cursor):
  ...
  - **Text A2** — "Good! Now move the sliders down as you exhale." Fades in over 1 second (`DIAG_FADE_IN_MS`) from 0, then tracks continuously: opacity = `lv`, fading out 1→0 as the user drags back down. When `lv <= DIAG_EDGE_THRESHOLD` (0.02), Text A2 is effectively invisible and Text B1 begins.
  - **Text B1** — "Wonderful! Now keep moving the sliders up and down with your breath." Fades in over 1 second. Once fully visible, stays up for **1 full breath cycle** — detected as 2 left-slider direction reversals ("strokes", `DIAG_CYCLE_STROKES = 2`, same 8%-deadband algorithm as the Vertical layout's oscillation counter, tracked in `diagDirectionRef`/`diagExtremeRef`/`diagReversalCountRef`) — then fades out over 2 seconds (`DIAG_FADE_OUT_MS`).
...
- **Slider Layouts screen** (`SliderLayoutsScreen.jsx`) — 2 selectable cards, **Vertical** / **Diagonal** (checkmark on the active one), same `optionBtn(selected)` pattern as Shape/Color Options. Persisted via `sliderLayout` (see Slider controls section). Single **Home** nav button (bottom center) returns to the hub.
- Navigation from the hub: Home → Personalize → Shapes or Colors; Home → Select Mode → (starts an experience); Home → Slider Layouts.
- All nav buttons are consolidated into stacked bottom-center groups — no top-corner buttons on any screen (except the Diagonal experience-screen nav, see below). Gap between buttons: 20px. Positioned at `bottom: 16, left: '50%', transform: 'translateX(-50%)'`.
