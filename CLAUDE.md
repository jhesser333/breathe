...
- Pool: 3 slots, single type
- **Road mesh** (Option E only, not C): a horizontal floor plane fills the gap between consecutive gates. Width (X) = the gate's interior opening (2 × inner-hole radius × `GATE_SCALE[0]`); position Y = `GATE_Y` (matches the Gates); length (Z) stretches dynamically so each end is pinned to the Z of the gate it borders. Tracked via an independent "checkpoint" list (one per gate spawn) decoupled from the gate-mesh pool's own recycling, plus a virtual anchor pinned at `DESPAWN_Z` (the same z where gates despawn off the bottom of the screen) — so the road always spans continuously from that screen-bottom anchor up to the lead gate, beginning the moment the very first gate spawns, with no gaps/popping as gates are recycled. A checkpoint (and the road segment trailing it) is only dropped once it scrolls past `DESPAWN_Z`. Material: `color`/`emissive` = `gateColor`, `emissiveIntensity` 0.5, base opacity 0.25 multiplied by a V-shaped alpha gradient (full at each end, 0 at the midpoint) via `onBeforeCompile`, further multiplied by each boundary's own fade-*in* only (not fade-out) — so the road ramps in cleanly with a newly spawned gate but does not dim when a gate fades out passing the Morph.
