...
- **Select Mode screen** (`SelectModeScreen.jsx`) is the app's home/initial screen (`screen` state starts at `'selectMode'`) — the "Modes" heading + 4 mode cards (Basic / Paced Breathing / Slowing Down / Box Breathing), plus two nav buttons: **Slider Layouts** pinned top-left (`top:16, left:16`, → Slider Layouts screen) and **Personalize** pinned bottom-center (`bottom:16, left:'50%', transform:'translateX(-50%)'`, → Personalize screen). There is no separate Home screen/hub — `HomeScreen.jsx` was removed and Select Mode absorbed its two entry points.
...
src/
  ...
  SelectModeScreen.jsx      — app's home screen ("Modes" heading, 4 mode cards); Slider Layouts nav button top-left, Personalize nav button bottom-center
