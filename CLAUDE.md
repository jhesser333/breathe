...
- Pool: 3 slots, single type
- **Road mesh** (Option E only, not C): two thin cylindrical "rails" (like train tracks), not a single plane. Each rail sits at X = ±(gate's interior inner-hole half-width − `RAIL_GAP`), inset from the gate's interior edge so it doesn't touch it; radius `RAIL_RADIUS`; position Y = `GATE_Y` (matches the Gates); length (Z) stretches dynamically so each end is pinned to the Z of the gate/anchor it borders (only the length scales — radius stays constant regardless of gate spacing). Boundaries = a virtual **leading** anchor pinned at `SPAWN_Z` (previews the segment beyond the upcoming gate before that gate has actually spawned, rather than only revealing it once the Morph passes through the current gate) + every live checkpoint (one per gate spawn, tracked independently of the gate-mesh pool's own recycling) + a virtual **trailing** anchor pinned at `DESPAWN_Z` (the same z where gates despawn off the bottom of the screen). This yields up to 3 simultaneous rail segments — the upcoming preview segment, the segment the Morph is currently on, and the already-passed trailing segment — so the rails span continuously from `DESPAWN_Z` up through `SPAWN_Z` with no gaps/popping as gates are recycled. A checkpoint (and the segments touching it) is only dropped once it scrolls past `DESPAWN_Z`. Material: `color`/`emissive` = `gateColor`, `emissiveIntensity` 0.5, base opacity `RAIL_ALPHA` (0.2) multiplied by a V-shaped alpha gradient along each rail's length (full `RAIL_ALPHA` at each end, 0 at the midpoint) via `onBeforeCompile`, further multiplied by each boundary's own fade-*in* only (not fade-out) — so a segment ramps in cleanly as its leading boundary appears, but does not dim when a gate fades out passing the Morph. Virtual anchors always contribute `fadeIn: 1`.
