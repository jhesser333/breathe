...
- Pool: 3 slots, single type
- **Road mesh** (Option E only, not C): railroad ties (`RoundedBox`, args `TIE_ARGS`, radius `TIE_RADIUS`), not rails or a plane. Each tie spans the full track width in X (`TIE_WIDTH_X` = 2 × (gate's interior inner-hole half-width − `TIE_GAP`), i.e. just touching where rails would sit if they existed) and is tiny in Y/Z (`TIE_HEIGHT_Y`/`TIE_DEPTH_Z`); position Y = `GATE_Y` (matches the Gates). Boundaries work exactly as the prior rail design did: a virtual **leading** anchor pinned at `SPAWN_Z` (previews the segment beyond the upcoming gate before it has actually spawned) + every live checkpoint (one per gate spawn, independent of the gate-mesh pool's recycling) + a virtual **trailing** anchor pinned at `DESPAWN_Z`. This yields up to 3 simultaneous segments (upcoming preview, the segment the Morph is currently on, and the already-passed trailing segment). Within each segment there are always exactly `TIES_PER_SEGMENT` (6) ties: one sits exactly at the segment's leading boundary (a real gate's Z, or a virtual anchor's Z), and 5 more equally spaced before the next boundary — so as gate spacing changes (e.g. Slowing Down's ramp), the tie *count* per gate-to-gate span stays fixed at 6 while their spacing stretches or shrinks. Each tie's opacity = `TIE_ALPHA` (0.2) × a V-shaped gradient by its fractional position within the segment (full alpha at the leading boundary, 0 at the segment midpoint, rising again toward the next boundary — computed directly from the tie's index, no shader needed) × the segment boundaries' fade-*in* only (not fade-out, so ties don't dim when a gate fades out passing the Morph). Virtual anchors always contribute `fadeIn: 1`. Material: `color`/`emissive` = `gateColor`, `emissiveIntensity` 0.5.
