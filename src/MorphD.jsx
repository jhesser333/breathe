...
const BOX_HALF = 0.5              // half-extent of the unit RoundedBox (args [1,1,1])
...
function sampleBoxPositions(count, half) {
  const positions = new Float32Array(count * 3)
  for (let i = 0; i < count; i++) {
    // Uniform pick of one of the 6 faces, then a random point on that face.
...
export default function MorphD({ leftVal, rightVal, palette }) {
  ...
  const { material, fresnelUniforms } = useMemo(() => {
    ...
    mat.customProgramCacheKey = () => `fresnel-morph-d-${palette.morphEmissive}`
...
\n` + shader.fragmentShader
  ...
  const sparkleAttrs = useMemo(() => {
    const positions = sampleBoxPositions(PARTICLE_COUNT, BOX_HALF)
  ...
  const flowAttrs = useMemo(() => {
    // Unscaled anchor points on the unit box -- used only at spawn time to
    ...
    const basePositions = sampleBoxPositions(PARTICLE_COUNT_2, BOX_HALF)
